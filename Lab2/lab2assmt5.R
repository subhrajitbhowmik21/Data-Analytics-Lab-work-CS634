...
#Find distributive measures and algebraic measures from the results of 4
COUNT=length(arr1)+length(arr2)+length(arr3)+length(arr4)
print(paste("Overall count=",COUNT))
SUM=Reduce("+",arr1)+Reduce("+",arr2)+Reduce("+",arr3)+Reduce("+",arr4)
print(paste("Overall Sum=",SUM))
MEAN=(mean(arr1)+mean(arr2)+mean(arr3)+mean(arr4))/4
print(paste("Overall mean=",MEAN))
